...
At the Step 1 reset, Task A had a bounded proxy/approximate execution history
built on:
...
That historical stack is preserved as execution history. It is not the final
full-STRIDE evidence layer, and it does not override the current
implementation status recorded in `docs/state.md`.

This document names full STRIDE as the formal rewiring target. Current
implementation status, including the bounded first-pass PyTorch/AdamW
full-estimator support envelope, explicit non-`ok` statuses, compatibility
fallback boundary, and remaining input-coverage work, is owned by
`docs/state.md`.
...
| Block 0 | Must be rerun on summaries derived from the rewired full-STRIDE surface rather than on preserved proxy-history evidence layers. |
| Block 1 | Must be rerun on full STRIDE patient plus cohort outputs, not only on preserved proxy-history summary exports. |
...
- Block 2 remains robustness, but after the refactor it must validate the full
  evidence stack rather than only preserved proxy-history summary surfaces.
- Block 3 remains method validation, but it must be rebuilt against full
  ...
  ablations must remove or zero the corresponding term and refit `A/d/e`
  under the same deterministic initialization, optimizer protocol, resolved
  evidence blocks, and rerun-specific semi-synthetic realization, without
  reweighting retained objective terms or masking fitted outputs. It must treat
  ...
  authority from preserved proxy-history packets.
- References to a future full-STRIDE Task A evidence stack are migration
  targets for artifacts that have not been rerun through the current contract.
  They must not relabel preserved Task A packets as current full-STRIDE
  evidence, and they must defer implementation-status claims for
  `fit_stride(...)` to `docs/state.md`.
