...
existing cache. Neither layer consumes prepare outputs, descriptive-atlas
artifacts, old suitability outputs, or passed bundles; neither emits biological
interpretation or downstream execution decisions.
...
from .metrics import build_block0_calibration_frames
...
from .writers import write_block0_analysis_outputs, write_block0_execution_outputs
    ...
    if real_records:
        ...
        try:
            ...
            _append_progress_event(
                ...
                status="ok",
                duration_seconds=duration_seconds,
                warning_summary=_fit_warning_summary(real_result),
            )
            _append_stage_progress_event(
                progress_path,
                stage="real_fit_extract",
...
def _guard_no_existing_analysis_outputs(output_root: Path) -> None:
    existing_outputs = tuple(
        filename for filename in _BLOCK0_ANALYSIS_OUTPUTS if (output_root / filename).exists()
    )
    if existing_outputs:
        raise ContractError(
            "Block 0 analysis output dir contains existing stale output artifacts; "
            "choose an empty output dir or remove stale outputs before rerun: "
            f"{existing_outputs}"
        )
...
def _append_progress_event(
    ...
    *,
    ...
    if warning_summary is not None:
        ...
        payload["warnings"] = list(warning_summary.get("warnings", ()))
    with progress_path.open("a", encoding="utf-8") as handle:
        json.dump(payload, handle, sort_keys=True)
        handle.write("\n")
        handle.flush()
