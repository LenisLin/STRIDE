...
- `deferred_boundaries.md` records which Task A surfaces remain task-local or deferred after the Block 0 calibration boundary.
...
- Block 0 freezes one STRIDE-native real-vs-null execution cache:
  `TC-IM` versus a within-patient count-preserving `TC/IM` domain-label
  permutation null. Cache-derived family-summary calibration analyses are
  separate from fit execution.
